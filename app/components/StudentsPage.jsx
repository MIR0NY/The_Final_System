...
import StudentFullInfoPage from './StudentFullInfoPage'; // Import the new StudentFullInfoPage
...
export default function StudentsPage({
  ...
  const [isModalOpen, setIsModalOpen] = useState(false); // For student add/edit modal
  const [editStudentId, setEditStudentId] = useState(null); // For student add/edit modal
  const [formState, setFormState] = useState({ // For student add/edit modal
    id: '',
    name: '',
    class: '',
    section: '',
  ...
  // State for the new Full Info View
  const [showFullInfoView, setShowFullInfoView] = useState(false);
  const [selectedStudentForFullInfo, setSelectedStudentForFullInfo] = useState(null);

  // State for the Payment Modal (controlled by StudentsPage and passed to StudentFullInfoPage)
  const [isPaymentModalOpen, setIsPaymentModalOpen] = useState(false);
  const [prefillStudentIdForPayment, setPrefillStudentIdForPayment] = useState(null);
  ...
  const calculateStudentDueMonths = useCallback((student, allPayments) => {
      ...
      // Function to open the student add/edit modal (local to StudentsPage)
      const openStudentEditModal = useCallback((student = null) => {
        if (student) {
      ...
      const handleStudentSubmit = async (e) => {
        ...
        try {
          ...
          if (apiResponse.ok) {
            ...
            closeStudentEditModal(); // Use local close function
            refreshData();
            // If we're in the full info view, also update the selected student state
            if (showFullInfoView && selectedStudentForFullInfo && selectedStudentForFullInfo.id === payload.id) {
                // Re-find the updated student from the refreshed students list
                const updatedStudent = students.find(s => s.id === payload.id);
                if (updatedStudent) {
                    setSelectedStudentForFullInfo(updatedStudent);
                }
            }
          } else {
      ...
      // --- Functions to control PaymentModal ---
      const handleOpenPaymentModal = useCallback((studentId = null) => {
        setPrefillStudentIdForPayment(studentId);
        setIsPaymentModalOpen(true);
      }, []);

      const handleClosePaymentModal = useCallback(() => {
        setIsPaymentModalOpen(false);
        setPrefillStudentIdForPayment(null);
      }, []);

      const handlePaymentSuccess = useCallback(() => {
        refreshData(); // Trigger data refresh in parent App component
      }, [refreshData]);
      // --- End Functions to control PaymentModal ---


      // --- New functions for Full Info View ---
      const handleViewFullInfo = useCallback((student) => {
        setSelectedStudentForFullInfo(student);
        setShowFullInfoView(true);
      }, []);

      const handleBackToStudents = useCallback(() => {
        setShowFullInfoView(false);
        setSelectedStudentForFullInfo(null);
      }, []);
      // --- End New functions for Full Info View ---


      const StudentCard = useCallback(({ student, handleViewFullInfoFromCard }) => {
        ...
        return (
          ...
          <div className="bg-white rounded-lg shadow-md p-4 space-y-2 border border-gray-200 h-[280px] flex flex-col justify-between">
            <div className="flex justify-between items-start flex-shrink-0">
              ...
              {/* REMOVED Add Payment and Edit buttons from here */}
            </div>
            ...
            <div className="flex justify-end pt-2 border-t mt-2">
                <button
                    onClick={() => handleViewFullInfoFromCard(student)}
                    className="btn-info text-sm py-1 px-3 rounded-md flex items-center whitespace-nowrap"
                >
                    <i className="fas fa-info-circle mr-1"></i> View Full Info
                </button>
            </div>
          </div>
        );
      }, [payments, currentUser, calculateStudentDueMonths, findLastPaidMonth, calculateTotal]);
      ...
      return (
        <section id="page-students" className="space-y-6 flex flex-col h-full">
          {showFullInfoView ? (
            // Render the full student info page
            <StudentFullInfoPage
              student={selectedStudentForFullInfo}
              payments={payments}
              onBack={handleBackToStudents}
              openEditStudentModal={openStudentEditModal} // Pass StudentsPage's openModal for editing
              openPaymentModal={handleOpenPaymentModal} // Pass StudentsPage's function to open payment modal
              currentUser={currentUser}
              showCustomAlert={showCustomAlert}
              refreshData={refreshData} // Pass refreshData for StudentFullInfoPage
            />
          ) : (
            // Render the main student list view
            <>
              <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <h2 className="text-3xl font-bold text-gray-800">Student Details</h2>
                {canEditStudents && (
                  <button
                    onClick={() => openStudentEditModal()} // Use local open function
              ...
              <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200 flex flex-col flex-grow min-h-0">
                ...
                {filteredStudents.length > 0 ? (
                  <div
                    ...
                    className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-4 flex-grow overflow-y-auto"
                  >
                    {filteredStudents.map(student => (
                      <StudentCard key={student.id} student={student} handleViewFullInfoFromCard={handleViewFullInfo} />
                    ))}
                  </div>
                ) : (
          ...
          {/* Student Add/Edit Modal (always render but controlled by isModalOpen state) */}
          {isModalOpen && (
              ...
              <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl p-8 overflow-y-auto max-h-screen">
                ...
                <form onSubmit={handleStudentSubmit}>
                  ...
                  <div className="mt-8 flex justify-end space-x-4">
                    <button type="button" onClick={closeStudentEditModal} className="bg-gray-200 text-gray-800 font-bold py-2 px-4 rounded-lg hover:bg-gray-300">Cancel</button>
          ...
          {/* Payment Modal Component (rendered conditionally based on state) */}
          <PaymentModal
            isOpen={isPaymentModalOpen}
            onClose={handleClosePaymentModal}
            initialStudentId={prefillStudentIdForPayment} // Pass student ID for pre-filling
            students={students}
            currentUser={currentUser}
            showCustomAlert={showCustomAlert}
